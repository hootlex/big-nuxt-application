...
const traverse = async (componentName, component, componentNamePrefix) => {
    ...
    if (!component.dependencies) {
        return await createTemplate(component.template, componentNamePrefix + componentName, [], outDir, component.data || emptyObjFunc)

    }

    if (component.dependencies) {

        let deps = await Promise.all(Object.entries(component.dependencies).map(c => traverse(...c, componentNamePrefix)))

        return await createTemplate(component.template, componentNamePrefix + componentName, deps, outDir, component.data || emptyObjFunc)

    }

};

const createTemplate = (template, componentName, dependencies, outDir, componentData) => {
...
const createIndex = (moduleName, links) => {
    ...
    return createTemplate(indexTemplatePath, 'index', [], modulePath, () => ({ links }))
}
...
(async () => {
    ...
    for (const mod of seeds.modules) {
        ...
        for (const page of seeds.pages) {
            ...
            for (const [componentName, component] of Object.entries(seeds.components)) {

                pageDeps.push(await traverse(componentName, component, componentNamePrefix));
